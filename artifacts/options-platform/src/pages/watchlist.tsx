...
import {
  ArrowUpRight, ArrowDownRight, Bookmark, Plus, Trash2,
  ChevronUp, ChevronDown, ChevronsUpDown, LineChart, Search,
...
export default function WatchlistPage() {
  ...
  const filtered = useMemo(() => {
    ...
    list.sort((a, b) => {
      ...
      switch (sortKey) {
        ...
        case "change":      av = (a as any).changePercent ?? 0; bv = (b as any).changePercent ?? 0; break;
        case "ivRank":      av = (a as any).ivRank ?? 0; bv = (b as any).ivRank ?? 0; break;
        case "techStrength":av = (a as any).technicalStrength ?? 0; bv = (b as any).technicalStrength ?? 0; break;
        case "score":       av = (a as any).opportunityScore ?? 0; bv = (b as any).opportunityScore ?? 0; break;
        case "addedAt":     av = a.addedAt; bv = b.addedAt; break;
        default:            av = 0; bv = 0;
      }
  ...
  const avgChange = watchlist.length
    ? watchlist.reduce((s, w) => s + ((w as any).changePercent ?? 0), 0) / watchlist.length
    : 0;
  ...
  const colHeader = (label: string, col: SortKey, align: "left" | "right" = "left") => (
    <th
      ...
      style={{
        ...
        color: sortKey === col ? "hsl(var(--foreground))" : "hsl(var(--muted-foreground))",
        textAlign: align, cursor: "pointer", whiteSpace: "nowrap", userSelect: "none",
        borderBottom: "1px solid rgba(255,255,255,0.06)",
      }}
    >
      <span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>
        {align === "right" && <SortIcon col={col} sortKey={sortKey} dir={sortDir} />}
        {label}
        {align === "left" && <SortIcon col={col} sortKey={sortKey} dir={sortDir} />}
      </span>
    </th>
  );
    ...
    <ScrollArea className="h-full w-full" style={{ background: "hsl(0 0% 4%)" }}>
      <div style={{ padding: isMobile ? "16px 12px 80px" : "28px 28px 60px", maxWidth: 1200, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: isMobile ? 16 : 24, gap: 12, flexWrap: "wrap" }}>
          ...
          {/* Add ticker form */}
          <form onSubmit={handleAdd} style={{ display: "flex", gap: 7, alignItems: "center" }}>
            <input
              value={addInput}
              ...
              spellCheck={false}
              style={{
                width: 110, padding: "8px 12px", borderRadius: 8, fontSize: 13, fontWeight: 600,
                border: "1px solid rgba(255,255,255,0.12)", background: "rgba(255,255,255,0.05)",
                color: "hsl(var(--foreground))", outline: "none", letterSpacing: "0.05em",
              }}
            />
            <button
              type="submit"
              ...
              style={{
                ...
                border: "1px solid hsl(var(--primary) / 0.4)", background: "hsl(var(--primary) / 0.12)",
                color: "hsl(var(--primary))", fontSize: 13, fontWeight: 600, cursor: "pointer",
                opacity: (!addInput.trim() || adding) ? 0.5 : 1, transition: "all 0.12s",
        ...
        {/* Summary stats */}
        {watchlist.length > 0 && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: isMobile ? 8 : 12, marginBottom: isMobile ? 16 : 22 }}>
            {[
              {
                label: "GAINERS",
                value: gainers,
              ...
              },
              {
                ...
                color: avgChange >= 0 ? "hsl(var(--success))" : "hsl(var(--destructive))",
              },
              {
                label: "HIGH IV",
                value: highIv,
              ...
              <div key={stat.label} style={{
                padding: isMobile ? "10px 12px" : "14px 16px", borderRadius: 10,
        ...
        {watchlist.length > 0 && (
          <div style={{ position: "relative", marginBottom: 14 }}>
            ...
            <input
              ...
              style={{
                width: "100%", padding: "9px 14px 9px 34px", fontSize: 13, borderRadius: 8,
        ...
        {isLoading ? (
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {Array.from({ length: 5 }).map((_, i) => (
              <div key={i} style={{ height: 64, borderRadius: 8, background: "rgba(255,255,255,0.04)", animation: "pulse 1.4s infinite" }} />
            ))}
          </div>
        ) : watchlist.length === 0 ? (
                  ...
                  <div style={{ padding: "13px 14px" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
                      <div>
                        ...
                        <div style={{ fontSize: 11, color: "hsl(var(--muted-foreground))", marginTop: 2 }}>{item.name}</div>
                      </div>
                    ...
                    <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
                      {iv > 0 && (
                        <span style={{ fontSize: 10, fontWeight: 600, padding: "2px 7px", borderRadius: 4,
                          color: iv >= 60 ? "hsl(38 92% 50%)" : "hsl(var(--muted-foreground))",
                          background: iv >= 60 ? "hsl(38 92% 50% / 0.12)" : "rgba(255,255,255,0.05)" }}>
                          IV {Math.round(iv)}%
                        </span>
                      )}
                      ...
                      <div style={{ display: "flex", gap: 6, marginLeft: "auto" }}>
                        <button onClick={() => setLocation(`/scanner?symbol=${item.symbol}`)}
                        ...
                        <button onClick={() => handleRemove(item.id, item.symbol)}
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead style={{ background: "rgba(255,255,255,0.02)" }}>
                <tr>
                  ...
                  <th style={{ padding: "10px 14px", fontSize: 10, fontWeight: 600, letterSpacing: "0.06em", textTransform: "uppercase", color: "hsl(var(--muted-foreground))", textAlign: "left", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>Name</th>
                  {colHeader("Price", "price", "right")}
                  {colHeader("Change", "change", "right")}
                  {colHeader("IV Rank", "ivRank", "right")}
                  {colHeader("Tech Str.", "techStrength", "right")}
                  {colHeader("Score", "score", "right")}
                  <th style={{ padding: "10px 14px", fontSize: 10, fontWeight: 600, letterSpacing: "0.06em", textTransform: "uppercase", color: "hsl(var(--muted-foreground))", textAlign: "right", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>Actions</th>
                </tr>
              </thead>
              <tbody>
                    ...
                    >
                      <td style={{ padding: "13px 14px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          ...
                          {anyItem.setupType && (
                            <span style={{ fontSize: 9, fontWeight: 600, padding: "1.5px 5px", borderRadius: 3,
                              color: "hsl(var(--muted-foreground))", background: "rgba(255,255,255,0.06)" }}>
                              {anyItem.setupType}
                            </span>
                          )}
                        </div>
                      </td>
                      <td style={{ padding: "13px 14px", fontSize: 12, color: "hsl(var(--muted-foreground))", maxWidth: 200 }}>
                        <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", display: "block" }}>{item.name}</span>
                      </td>
                      <td style={{ padding: "13px 14px", textAlign: "right", fontSize: 13, fontWeight: 600, fontVariantNumeric: "tabular-nums" }}>
                        {formatCurrency(item.price)}
                      </td>
                      <td style={{ padding: "13px 14px", textAlign: "right" }}>
                        <span style={{ display: "inline-flex", alignItems: "center", gap: 3, fontSize: 12, fontWeight: 600,
                          color: isUp ? "hsl(var(--success))" : "hsl(var(--destructive))", fontVariantNumeric: "tabular-nums" }}>
                          {isUp ? <ArrowUpRight style={{ width: 12, height: 12 }} /> : <ArrowDownRight style={{ width: 12, height: 12 }} />}
                          {formatPercent(Math.abs(anyItem.changePercent ?? 0))}
                        </span>
                      </td>
                      <td style={{ padding: "13px 14px", textAlign: "right" }}>
                        {iv > 0 ? (
                          <span style={{ fontSize: 12, fontWeight: 600, fontVariantNumeric: "tabular-nums",
                            color: iv >= 60 ? "hsl(38 92% 50%)" : iv >= 40 ? "hsl(var(--primary))" : "hsl(var(--muted-foreground))" }}>
                            {Math.round(iv)}%
                          </span>
                      ...
                      <td style={{ padding: "13px 14px", textAlign: "right" }}>
                        <span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>
                          <span style={{ width: 36, height: 4, borderRadius: 99, background: "rgba(255,255,255,0.06)", overflow: "hidden", display: "inline-block" }}>
                        ...
                        <div style={{ display: "flex", gap: 6, justifyContent: "flex-end" }} onClick={e => e.stopPropagation()}>
                          <button
                            ...
                            style={{ display: "flex", alignItems: "center", gap: 5, padding: "5px 11px", borderRadius: 6,
                              ...
                              color: "hsl(var(--muted-foreground))", fontSize: 11, fontWeight: 500, cursor: "pointer",
                          ...
                          <button
                            ...
                            style={{ display: "flex", alignItems: "center", padding: "5px 8px", borderRadius: 6,
                              ...
                              color: "hsl(var(--muted-foreground))", cursor: "pointer", transition: "color 0.1s" }}
                            onMouseEnter={e => (e.currentTarget as HTMLButtonElement).style.color = "hsl(var(--destructive))"}
                            onMouseLeave={e => (e.currentTarget as HTMLButtonElement).style.color = "hsl(var(--muted-foreground))"}
                          >
                            <Trash2 style={{ width: 13, height: 13 }} />
